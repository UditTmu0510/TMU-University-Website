    ...
    public function store(Request $request)
{
    if ($request->isMethod('post')) {
        try {
            ...
            $prog_code = $prog_name ? $prog_name->prog_code : null;
            ...
            if ($request->hasFile('file_path')) {
                ...
                $directoryPath = public_path('pdf/syllabus/' . $acad_year);
                
                if (!is_dir($directoryPath)) {
                    mkdir($directoryPath, 0755, true);
                }

                $filePath = 'pdf/syllabus/' . $acad_year . '/' . $filename;
            ...
            if ($result) {
                ...
                Session::flash('error', 'Error in adding the syllabus');
            }
        ...
        $id_profile = Auth::user()->id;
        $profileData = User::find($id_profile);
        $colleges = Colleges::where('status', 1)
            ->where('for_programmes', 'Y')
            ->orderBy('cd_name', 'ASC')
            ->select('cd_name', 'cd_id')
            ->get();
    ...
    public function update(Request $request, string $id)
    {
        if ($request->isMethod('post')) {
            try {
                ...
                $acad_year = $validatedData['acad_year'];
                $status = $validatedData['status'];
                $file_path_value = $request->file_path_value;
                ...
                if ($request->hasFile('file_path')) {
                    ...
                    if ($file_path_value && Storage::disk('public')->exists($file_path_value)) {
                    ...
                    $file->move($directoryPath, $filename);
                    $syllabusdata['file_path'] = 'pdf/syllabus/'.$acad_year.'/'.$filename;
                }
                ...
                Session::flash('error', 'Validation Error in Updating Syllabus: ' . $e->getMessage());
                return redirect()->route('all.syllabus');
            } catch (Exception $e) {
                Session::flash('error', 'Error in Updating Syllabus: ' . $e->getMessage());
                return redirect()->route('all.syllabus');
            }
