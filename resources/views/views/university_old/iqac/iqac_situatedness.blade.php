@extends('layouts.university.institutional_values_with_sidebar')
@section('content')



<div class="col-12 col-md-9 col-lg-9 mt-3 mt-sm-0 pt-5">

  <div class="row mx-0">
    <h1 class="tmu-text-primary tmu-page-heading"><span>Inclusive Environmnet</span> <span>Activities</span></h1>

    <div class="container mb-3 mt-3">
      <hr class="horizontal-line">

      <button class="careers-accordion"> Inclusive Environmnet Activities<span class="icon">+</span></button>
      <div class="panel pt-3  active">
        <div class="row justify-content-center gutter-30 col-mb-5">
          <div class="col-sm-8">
            <h3 class="tmu-text-primary">CENTRE FOR JAIN STUDIES</h3>
            <p>Teerthanker Mahaveer University stands for humanism and is committed to the ideals of Lord Mahaveer - Right
              Philosophy, Right Knowledge and Right Conduct in all endeavors. The College for Jain Studies has been
              established to impart moral education, comparative studies of different philosophies, and comprehensive
              research in religious studies. The core principles of Jain philosophy - Non-Violence, Non-Absolutism and
              Non-Possessiveness may prove to be the foundation for world peace.</p>
          </div>
          <div class="col-sm-4 col-md-4">
            <div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
              <div class="team-image mt-1 mb-1">
                <img src="https://www.tmu.ac.in/img/facilities/institutional_values/centre_jain_studies.jpeg" alt="" style="height:250px;">
              </div>
              <div class="team-desc p-2">
                <div class="team-title pt-0">
                  <h4 class="fs-14">Centre for Jain Studies</h4><span class="fst-normal fs-12"><i class="fa-solid fa-location-dot"></i> TMU
                    Campus</span>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>
      <!-- Add the horizontal line here -->
      <hr class="horizontal-line">

      <button class="careers-accordion"> Parampara Edition 1<span class="icon">+</span></button>
      <div class="panel pt-3 ">
        <p>On the occasion of the 21st foundation day of <a href="https://www.tmu.ac.in/"> Teerthanker Mahaveer University</a>, Moradabad along with Azadi ka Amrit Mahotsav, the university organised a 3-day Cultural Fest - Parampara on 13th, 14th, and 15th July. The event was organised in association with Breathing Arts. The special chief guest at the event was Smt. Meenakshi Lekhi, Minister of State for External Affairs and Culture, Government of India.</p>

        <h3 class="tmu-text-primary">July 13, 2022</h3>
        <b>Pt. Vishwa Mohan Bhatt & Ustad Anwar Khan Manganiyar</b>
        <p><b>On the first day</b>, Pt. Vishwa Mohan Bhatt (Padma Bhushan, Padma Shri & Grammy award winner) and Rajasthani Sufi singer Ustad Anwar Khan Manganiyar (Padma Shri award winner). It was a pleasure for the listeners to imbibe the rich soothing music of Vishwa Mohan Bhatt along with the energetic and blustery melody of his son, Salil Bhatt. This was followed by performances in various ragas. The audience was completely delighted with the melodious song - Kesariya Balam Padharo Mhare Desh…performed by Ustad Anwar Khan Ji</p>

        <h3 class="tmu-text-primary">July 14, 2022</h3>
        <b>Nizami Bandhu</b>
        <p><b>On the Second day</b>, the group of famous qawwals- Ustad Chand Nizami, Shadab Faridi and Sohrab Faridi Nizami popularly known as the <a href="https://nizamibandhu.com/">Nizami Bandhu</a> made everyone fascinated by reciting the famous qawwali Kun Faya Kun..., the famous song from the film Rockstar. Chand Nizami’s nephew Shadab, Sohrab’s sons Kamran and Gulkhan and ten co-stars started the program by remembering Allah. Meanwhile, Kabir Das's famous bhajan Mann Laga Mora Fakiri Mein... was narrated in a new way, listening to which everyone was immersed in the winds of devotion.</p>

        <h3 class="tmu-text-primary">July 15, 2022</h3>
        <b>Dr. Sonal Mansingh</b>
        <p><b>On the Third day</b>, the group of famous qawwals- Ustad Chand Nizami, Shadab Faridi and Sohrab Faridi Nizami popularly known as the Nizami Bandhu made everyone fascinated by reciting the famous qawwali Kun Faya Kun..., the famous song from the film Rockstar. Chand Nizami’s nephew Shadab, Sohrab’s sons Kamran and Gulkhan and ten co-stars started the program by remembering Allah. Meanwhile, Kabir Das's famous bhajan Mann Laga Mora Fakiri Mein... was narrated in a new way, listening to which everyone was immersed in the winds of devotion.</p>
      </div>
      <hr class="horizontal-line">

      <button class="careers-accordion"> ROCK-ON (A CULTURAL FEST)<span class="icon">+</span></button>
      <div class="panel pt-3 ">
        <p>The university has been organizing cultural extravaganza Rock-On. In the past eminent performers like Shaan,
          Sunidhi Chauhan, Sukhwinder Singh, Mika, Sonu Nigam, Band of Boys, Amaal Malik &Armaan Malik and many others
          have graced the shows.</p>
      </div>
      <hr class="horizontal-line">

      <button class="careers-accordion"> SPICMACAY <span class="icon">+</span></button>
      <div class="panel pt-3 ">
        <p>In association with SPIC MACAY the university regularly organizes cultural & artistic activities.</p>
        <table class="table table-bordered table-striped">
          <thead>
            <th>#</th>
            <th>Date</th>
            <th>Time</th>
            <th>Module Type</th>
            <th>Artists Name</th>
          </thead>
          <tbody>
            <tr>
              <td>1</td>
              <td>13/09/2015</td>
              <td>8:00 PM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>2</td>
              <td>14/09/2015</td>
              <td>10:00 AM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>3</td>
              <td>14/09/2015</td>
              <td>2:00 PM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>4</td>
              <td>15/09/2015</td>
              <td>10:00 AM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>5</td>
              <td>15/09/2015</td>
              <td>2:00 PM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>6</td>
              <td>16/09/2015</td>
              <td>10:00 AM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>7</td>
              <td>16/09/2015</td>
              <td>2:00 PM</td>
              <td>Vocal</td>
              <td>Pt. Bhola Nath Mishra</td>
            </tr>
            <tr>
              <td>8</td>
              <td>27/10/2015</td>
              <td>10:30 PM</td>
              <td>Goti Pok Fock</td>
              <td>Sh. Vijoy Sahu & Group</td>
            </tr>
            <tr>
              <td>9</td>
              <td>27/10/2015</td>
              <td>10:00 AM</td>
              <td>Goti Pok Fock</td>
              <td>Sh. Vijoy Sahu & Group</td>
            </tr>
            <tr>
              <td>10</td>
              <td>29/10/2015</td>
              <td>2:00 PM</td>
              <td>Goti Pok Fock</td>
              <td>Sh. Vijoy Sahu & Group</td>
            <tr>
              <td>11</td>
              <td>01/12/2016</td>
              <td>12:00 PM</td>
              <td>Vocal</td>
              <td>Sh Rupan Sarkar</td>
            </tr>
            <tr>
              <td>12</td>
              <td>01/12/2016</td>
              <td>3:00 PM</td>
              <td>Vocal</td>
              <td>Sh Rupan Sarkar</td>
            </tr>
            <tr>
              <td>13</td>
              <td>26/03/2017</td>
              <td>4:00 PM</td>
              <td>Chhau Dance</td>
              <td>Sh Chinibas Mahto & Group</td>
            </tr>
            <tr>
              <td>14</td>
              <td>27/03/2017</td>
              <td>10:00 AM</td>
              <td>Chhau Dance</td>
              <td>Sh Chinibas Mahto & Group</td>
            </tr>
            <tr>
              <td>15</td>
              <td>27/03/2017</td>
              <td>2:00 AM</td>
              <td>Chhau Dance</td>
              <td>Sh Chinibas Mahto & Group</td>
            </tr>
            <tr>
              <td>16</td>
              <td>28/03/2017</td>
              <td>11:00 AM</td>
              <td>Chhau Dance</td>
              <td>Sh Chinibas Mahto & Group</td>
            </tr>
            <tr>
              <td>17</td>
              <td>28/03/2017</td>
              <td>2:00 PM</td>
              <td>Chhau Dance</td>
              <td>Sh Chinibas Mahto & Group</td>
            </tr>
            <tr>
              <td>18</td>
              <td>13/11/2017</td>
              <td>2:00 PM</td>
              <td>Vocal</td>
              <td>Sh. Mahua Shankar</td>
            </tr>
            <tr>
              <td>19</td>
              <td>14/11/2017</td>
              <td>9:00 AM</td>
              <td>Vocal</td>
              <td>Sh. Mahua Shankar</td>
            </tr>
            <tr>
              <td>20</td>
              <td>21/11/2018</td>
              <td>10:00 AM</td>
              <td>Rajasthan Fock Dance</td>
              <td>Sh. Rose Khan & Group</td>
            </tr>
            <tr>
              <td>21</td>
              <td>21/11/2018</td>
              <td>2:00 PM</td>
              <td>Rajasthan Fock Dance</td>
              <td>Sh. Rose Khan & Group</td>
            </tr>
            <tr>
              <td>22</td>
              <td>22/11/2018</td>
              <td>2:00 PM</td>
              <td>Rajasthan Fock Dance</td>
              <td>Sh. Rose Khan & Group</td>
            </tr>
            <tr>
              <td>23</td>
              <td>27/11/2018</td>
              <td>10:00 AM</td>
              <td>Rajasthan Fock Dance</td>
              <td>Sh. Rose Khan & Group</td>
            </tr>
            <tr>
              <td>24</td>
              <td>27/11/2018</td>
              <td>2:00 PM</td>
              <td>Rajasthan Fock Dance</td>
              <td>Sh. Rose Khan & Group</td>
            </tr>
            <tr>
              <td>25</td>
              <td>18/10/2019</td>
              <td>10:00 AM</td>
              <td>Kathakali</td>
              <td>M. Amal Jithe & Group</td>
            </tr>
            <tr>
              <td>26</td>
              <td>18/10/2019</td>
              <td>2:00 PM</td>
              <td>Kathakali</td>
              <td>M. Amal Jithe & Group</td>
            </tr>
            <tr>
              <td>27</td>
              <td>18/10/2019</td>
              <td>10:00 AM</td>
              <td>Kathakali</td>
              <td>M. Amal Jithe & Group</td>
            </tr>
          </tbody>
        </table>

        <div class="row">
          <div class="col-sm-4 col-md-4">
            <div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
              <div class="team-image">
                <img src="	https://www.tmu.ac.in/img/facilities/institutional_values/spic_macay1.jpg" alt="" style="height:250px;">
              </div>
              <div class="team-desc p-2">
                <div class="team-title pt-0">
                  <h4 class="fs-14">Centre for Jain Studies</h4><span class="fst-normal fs-12"><i class="fa-solid fa-location-dot"></i> TMU
                    Campus</span>
                </div>
              </div>
            </div>
          </div>

          <div class="col-sm-4 col-md-4">
            <div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
              <div class="team-image">
                <img src="https://www.tmu.ac.in/img/facilities/institutional_values/spic_macay2.jpg" alt="" style="height:250px;">
              </div>
              <div class="team-desc p-2">
                <div class="team-title pt-0">
                  <h4 class="fs-14">Centre for Jain Studies</h4><span class="fst-normal fs-12"><i class="fa-solid fa-location-dot"></i> TMU
                    Campus</span>
                </div>
              </div>
            </div>
          </div>

          <div class="col-sm-4 col-md-4">
            <div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
              <div class="team-image">
                <img src="https://www.tmu.ac.in/img/facilities/institutional_values/spic_macay3.jpg" alt="" style="height:250px;">
              </div>
              <div class="team-desc p-2">
                <div class="team-title pt-0">
                  <h4 class="fs-14">Centre for Jain Studies</h4><span class="fst-normal fs-12"><i class="fa-solid fa-location-dot"></i> TMU
                    Campus</span>
                </div>
              </div>
            </div>
          </div>
        </div>

        <p>Apart from above, University has celebrated National Festival, Commemorative Days and Cultural & Regional Celebrations etc. on regular basis.</p>
        <div class="button-container"><a href="#" class="tmu-btn btn-1 py-2 my-3">
            <i class="bi-book"></i> View Images</a>
        </div>
        <table class="table table-bordered table-striped mt-3">
          <thead>
          </thead>
          <tbody>
            <tr>
              <td colspan="2" style="font-weight:bold; text-align:center;">National Festival</td>
            </tr>
            <tr>
              <th>Republic Day</th>
              <th>Independence Day</th>
            </tr>
            <tr>
              <td colspan="2" style="font-weight:bold; text-align:center;">JAYANTI</td>
            </tr>
            <tr>
              <td>Gandhi Jayanti</td>
              <td>Swami Vivekanand Jayanti</td>
            </tr>
            <tr>
              <td>Sardar Vallabhbhai Patel Jayanti</td>
              <td>Mahavir Jayanti</td>
            </tr>
            <tr>
              <td>Subhash Chandra Bose Jayanti</td>
              <td>University Establishment Day</td>
            </tr>
            <tr>
              <td>Atal Bihari Bajpai Jayanti</td>
              <td>National Youth Day</td>
            </tr>
            <tr>
              <td>Dr. B.R. Ambedkar Jayanti</td>
              <td>Dr. APJ Abdul Kalam Jayanti</td>
            </tr>
            <tr>
              <td colspan="2" style="font-weight:bold; text-align:center;">CULTURAL & REGIONAL CELEBRATIONS</td>
            </tr>
            <tr>
              <th>Yoga & Meditation</th>
              <th>Kavi Sammellan</th>
            </tr>
            <tr>
              <td>Paryushan Parva</td>
              <td>Basant Panchami</td>
            </tr>
            <tr>
              <td>Dandiya</td>
              <td>Food Festival</td>
            </tr>
            <tr>
              <td>Lohri Celebration</td>
              <td>Janmashtmi</td>
            </tr>
            <tr>
              <td>Holi-Diwali</td>
              <td>Workshop on Communal Harmony</td>
            </tr>
            <tr>
              <td>Dharmik Pravachan</td>
              <td>Pratibha Samman Samaroh</td>
            </tr>
            <tr>
              <td colspan="2" style="font-weight:bold; text-align:center;">TRIBUTE TO MARTYRS</td>
            </tr>
            <tr>
              <th>Maa Kasam</th>
              <th>Ek Deep Shaheedo Ke Naam</th>
            </tr>
            <tr>
              <td>Yaad Karo Qurbani</td>
              <td>Shaheedon Ko Salam</td>
            </tr>
            <tr>
              <td>Tribute to Martyrs</td>
              <td>Ek Shaam Azaadi Ke Naam</td>
            </tr>
            <tr>
              <td>Tribute to Pulwama CRPF Jawans</td>
              <td></td>
            </tr>
            <tr>
              <td colspan="2" style="font-weight:bold; text-align:center;">Others</td>
            </tr>
            <tr>
              <th>Blood Donation Camp</th>
              <th>Commando Training</th>
            </tr>
            <tr>
              <td>Sports & Games</td>
              <td>Nasha Mukt Bharat Campaign</td>
            </tr>
            <tr>
              <td>Scout & Guide Camp</td>
              <td>Contribution to PM Relief Fund</td>
            </tr>
            <tr>
              <td>Donation to Ram Mandir Ayodhaya</td>
              <td>Quit India Movement</td>
            </tr>
            <tr>
              <td>Vote for Nation</td>
              <td>Fit India</td>
            </tr>
          </tbody>
        </table>

        <button class="tmu-btn btn-1 py-2 mt-4"><i class="bi-book"></i> View Images</button>
      </div>
      <hr class="horizontal-line">

    </div>
  </div>
</div>
</div>
</div>
</div>
</div>
</section><!-- #content end -->


@endsection