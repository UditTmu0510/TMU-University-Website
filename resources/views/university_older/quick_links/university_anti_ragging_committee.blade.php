@extends('layouts.university.departments.quicklinks_with_sidebar')
@section('content')

<div class="main-content">
    <div class="container">
        <div class="row mx-0">
            <h1 class="tmu-text-primary tmu-page-heading pt-md-5"><span> University Anti </span><span> Ragging Committee</span>
            </h1>
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th class="text-center">Sl. No.</th>
                            <th>Name</th>
                            <th>Designation</th>
                            <th>Contact No.</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-center">1</td>
                            <td>Prof. Sushil Kumar Singh</td>
                            <td>Chairperson</td>
                            <td>9935070015</td>
                        </tr>

                        <tr>
                            <td class="text-center">2</td>
                            <td>Mr. Vipin Jain</td>
                            <td>Member</td>
                            <td>9837763888</td>
                        </tr>

                        <tr>
                            <td class="text-center">3</td>
                            <td>Dr. Manjula Jain</td>
                            <td>Member</td>
                            <td>6398245857</td>
                        </tr>

                        <tr>
                            <td class="text-center">4</td>
                            <td>Dr. R.K. Dwivedi</td>
                            <td>Member</td>
                            <td>9837771280</td>
                        </tr>

                        <tr>
                            <td class="text-center">5</td>
                            <td>Prof. Vipin Jain</td>
                            <td>Member</td>
                            <td>9917200216</td>
                        </tr>

                        <tr>
                            <td class="text-center">6</td>
                            <td>Prof. Nikhil Rastogi</td>
                            <td>Member</td>
                            <td>9258118526</td>
                        </tr>

                        <tr>
                            <td class="text-center">7</td>
                            <td>Prof. Jaslin M</td>
                            <td>Member</td>
                            <td>9568526444</td>
                        </tr>

                        <tr>
                            <td class="text-center">8</td>
                            <td>Mr. R.P. Gupta </td>
                            <td>Member</td>
                            <td>9639236666</td>
                        </tr>

                        <tr>
                            <td class="text-center">9</td>
                            <td>Ms. Sushma Bhadoria</td>
                            <td>Member</td>
                            <td>9927045580</td>
                        </tr>

                        <tr>
                            <td class="text-center">10</td>
                            <td>Dr. Madhusudan</td>
                            <td>Member</td>
                            <td>9837033001</td>
                        </tr>

                        <tr>
                            <td class="text-center">11</td>
                            <td>Dr. Aditya Sharma</td>
                            <td>Member Secretary</td>
                            <td>9837933666</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div class="container">
                <div class="row justify-content-center mb-5 mt-5">
                    <div class="col-md-8 mb-3 mb-sm-0" style="background-color: #001055;">
                        <div class="card border-light my-3">
                            <div class="card-body text-center">
                                <h3 class="card-title tmu-text-primary display-6"><span>ANTI RAGGING TOLL FREE</span></h3>
                                <p class="card-text h1 tmu-color">HELPLINE NO. 1800-180-5522 </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <h2 class="tmu-text-primary tmu-page-heading"><span>University Proctorial </span><span> Board
                    Committee</span></h2>
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <thead>
                        <th class="text-center">Sl. No.</th>
                        <th>Name</th>
                        <th>Designation</th>
                        <th>Contact No.</th>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-center">
                                1 </td>
                            <td>Prof. Sushil Kumar Singh</td>
                            <td>Chairperson</td>
                            <td>9935070015</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                2 </td>
                            <td>Dr. S.K. Jain</td>
                            <td>Proctor</td>
                            <td>9997168754</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                3 </td>
                            <td>Dr. Devendra Pal Singh</td>
                            <td>Proctor</td>
                            <td>9013666825</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                4 </td>
                            <td>Mr. Ravi Kumar</td>
                            <td>Proctor</td>
                            <td>9690444424</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                5 </td>
                            <td>Mr. Touheed Akhtar</td>
                            <td>Proctor</td>
                            <td>9568872799</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                6 </td>
                            <td>Prof. R.C. Tripathi</td>
                            <td>Proctor</td>
                            <td>9868232444</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                7 </td>
                            <td>Dr. Shivani Kaul</td>
                            <td>Proctor</td>
                            <td>9568232444</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                8 </td>
                            <td>Prof. Ram Niwas</td>
                            <td>Proctor</td>
                            <td>9548658247</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                9 </td>
                            <td>Prof. Rashmi Mahrotra</td>
                            <td>Proctor</td>
                            <td>9456886999</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                10 </td>
                            <td>Mr. Saurabh Battar</td>
                            <td>Proctor</td>
                            <td>9821339884</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                11 </td>
                            <td>Dr. Farha Deeba </td>
                            <td>Proctor</td>
                            <td>9045395032</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                12 </td>
                            <td>Dr. Chanchal Chawla</td>
                            <td>Proctor</td>
                            <td>9808358721</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                13 </td>
                            <td>Prof. Rajeev Pathak</td>
                            <td>Proctor</td>
                            <td>9336344974</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                14 </td>
                            <td>Prof. Mukesh Sikarwar</td>
                            <td>Proctor</td>
                            <td>6395867230</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                15 </td>
                            <td>Dr. Ankita Jain</td>
                            <td>Proctor</td>
                            <td>7936476348</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                16 </td>
                            <td>Ms. Anshu Chauhan</td>
                            <td>Proctor</td>
                            <td>9720008334</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                17 </td>
                            <td>Dr. Gulista Khan</td>
                            <td>Proctor</td>
                            <td>9639888358</td>
                        </tr>
                        <tr>
                            <td class="text-center">
                                - </td>
                            <td>Dr. M.P. Singh</td>
                            <td>Special Invitee</td>
                            <td>9568162444</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
</div>


@endsection