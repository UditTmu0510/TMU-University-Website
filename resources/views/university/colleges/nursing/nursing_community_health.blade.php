@extends('layouts.university.colleges.nursing_with_sidebar')
@section('content')


<div class="main-content">
	<div class="container">

		<section class="about-section bg-light mt-4">
			<div class="container">
				<div class="text-content p-4">
					<h1 class="tmu-text-primary tmu-page-heading"><span>Department of </span><span> Community Health Nursing </span></h1>
					<p class="about-p mb-3">The Department of Community Health Nursing takes initiative in fostering and strengthening the relationship of College with the community. The Department imparts quality education to the students and is committed to prepare them as efficient community health nurses.</p>
				</div>
				<div class="image-content text-center p-2">
					<img src="{{asset('/assets/img/college_departments/nursing/chn/banner.jpg')}}" alt="">
				</div>
				<div class="container p-3">
					<p class="about-p mb-4">The department creatively plans all its activities towards strengthening the humane and technical skills of students to enable them to provide quality service to individual, family and community. This department collaborates with community members to provide care, improve the health of children, adults and families. These collaborations are life-changing for the community as well as for students, who receive hands-on experience to become more adept at engaging and promoting health.</p>
					<h3 class="tmu-text-primary"><span>Our </span><span> Laboratories</span></h3>
					
					<div class="row justify-content-left gutter-30 col-mb-5 mb-5">
						<div class="col-sm-4 col-md-4">
							<div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
								<div class="team-image">
									<img src="{{asset('/assets/img/college_departments/nursing/chn/Lab1.jpg')}}" height="150px" alt="">
								</div>
								<div class="team-desc p-2">
									<div class="team-title pt-0">
										<h4 class="fs-14">Nutrition Lab</h4>
										<span class="fst-normal fs-12 mb-2"><i class="fa-solid fa-location-dot"></i> College of Nursing, TMU Campus</span>
										<a class="tmu-btn btn-2 m-0 py-1 px-2 fs-12 mb-4" href="{{asset('/assets/img/college_departments/nursing/chn/geotag/Lab1.jpg')}}" download>
											Download Geo Tag Image
										</a>
									</div>
								</div>
							</div>
						</div>

						<div class="col-sm-4 col-md-4">
							<div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
								<div class="team-image">
									<img src="{{asset('/assets/img/college_departments/nursing/chn/Lab2.jpg')}}" height="150px" alt="">
								</div>
								<div class="team-desc p-2">
									<div class="team-title pt-0">
										<h4 class="fs-14">Nutrition Lab </h4>
										<span class="fst-normal fs-12 mb-2"><i class="fa-solid fa-location-dot"></i> College of Nursing, TMU Campus</span>
										<a class="tmu-btn btn-2 m-0 py-1 px-2 fs-12 mb-4" href="{{asset('/assets/img/college_departments/nursing/chn/geotag/Lab2.jpg')}}" download>
											Download Geo Tag Image
										</a>
									</div>
								</div>
							</div>
						</div>

						<div class="col-sm-4 col-md-4">
							<div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
								<div class="team-image">
									<img src="{{asset('/assets/img/college_departments/nursing/chn/lab3.jpg')}}" height="150px" alt="">
								</div>
								<div class="team-desc p-2">
									<div class="team-title pt-0">
										<h4 class="fs-14">Communityn Health Nursing Lab</h4>
										<span class="fst-normal fs-12 mb-2"><i class="fa-solid fa-location-dot"></i> College of Nursing, TMU Campus</span>
										<a class="tmu-btn btn-2 m-0 py-1 px-2 fs-12 mb-4" href="{{asset('/assets/img/college_departments/nursing/chn/geotag/lab3.jpg')}}" download>
											Download Geo Tag Image
										</a>
									</div>
								</div>
							</div>
						</div>

						<div class="col-sm-4 col-md-4">
							<div class="team card shadow-sm border-0 h-shadow h-translatey-sm all-ts rounded-4 overflow-hidden">
								<div class="team-image">
									<img src="{{asset('/assets/img/college_departments/nursing/chn/lab4.jpg')}}" height="150px" alt="">
								</div>
								<div class="team-desc p-2">
									<div class="team-title pt-0">
										<h4 class="fs-14">Communityn Health Nursing Lab/h4>
										<span class="fst-normal fs-12 mb-2"><i class="fa-solid fa-location-dot"></i> College of Nursing, TMU Campus</span>
										<a class="tmu-btn btn-2 m-0 py-1 px-2 fs-12 mb-4" href="{{asset('/assets/img/college_departments/nursing/chn/geotag/lab4.jpg')}}" download>
											Download Geo Tag Image
										</a>
									</div>
								</div>
							</div>
						</div>
					</div>



					<h3 class="tmu-text-primary"><span>Labs </span><span> Equipments</span></h3>
					<div class="row">
						<div class="col-md-6">
							<table class="table table-bordered table-striped">
								<thead>
									<tr>
										<th class="text-center">Sl. No.</th>
										<th>Name of Equipments</th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td colspan="2" style="text-align:center"><strong>COMMUNITY LABORATORY</strong></td>
									</tr>
									<tr>
										<td class="text-center" sdval="1">1</td>
										<td>Artery Forcep Curved</td>
									</tr>
									<tr>
										<td class="text-center" sdval="2">2</td>
										<td>Artery Forcep Straight</td>
									</tr>
									<tr>
										<td class="text-center" sdval="3">3</td>
										<td>Betadine Solution</td>
									</tr>
									<tr>
										<td class="text-center" sdval="4">4</td>
										<td>Big Bowl</td>
									</tr>
									<tr>
										<td class="text-center" sdval="5">5</td>
										<td>Big Drum</td>
									</tr>
									<tr>
										<td class="text-center" sdval="6">6</td>
										<td>Blanket</td>
									</tr>
									<tr>
										<td class="text-center" sdval="7">7</td>
										<td>Community Bag</td>
									</tr>
									<tr>
										<td class="text-center" sdval="8">8</td>
										<td>Condom Training Model</td>
									</tr>
									<tr>
										<td class="text-center" sdval="9">9</td>
										<td>Cotton Roll</td>
									</tr>
									<tr>
										<td class="text-center" sdval="10">10</td>
										<td>Dettol Solution</td>
									</tr>
									<tr>
										<td class="text-center" sdval="11">11</td>
										<td>Disposable Syringes 10 CC</td>
									</tr>
									<tr>
										<td class="text-center" sdval="12">12</td>
										<td>Disposable Syringes 5CC</td>
									</tr>
									<tr>
										<td class="text-center" sdval="13">13</td>
										<td>Dropping Bottles</td>
									</tr>
									<tr>
										<td class="text-center" sdval="14">14</td>
										<td>Enema Can</td>
									</tr>
									<tr>
										<td class="text-center" sdval="15">15</td>
										<td>Fetoscope</td>
									</tr>
									<tr>
										<td class="text-center" sdval="16">16</td>
										<td>Glass Syringes</td>
									</tr>
									<tr>
										<td class="text-center" sdval="17">17</td>
										<td>Gloves</td>
									</tr>
									<tr>
										<td class="text-center" sdval="18">18</td>
										<td>Glucometer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="19">19</td>
										<td>Glucometer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="20">20</td>
										<td>Glucostrips</td>
									</tr>
									<tr>
										<td class="text-center" sdval="21">21</td>
										<td>Green Sheet</td>
									</tr>
									<tr>
										<td class="text-center" sdval="22">22</td>
										<td>Hydrogen Peroxide</td>
									</tr>
									<tr>
										<td class="text-center" sdval="23">23</td>
										<td>Inch Tape</td>
									</tr>
									<tr>
										<td class="text-center" sdval="24">24</td>
										<td>Infantometer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="25">25</td>
										<td>Kidney Tray Plastic</td>
									</tr>
									<tr>
										<td class="text-center" sdval="26">26</td>
										<td>Kidney Tray Steel Small</td>
									</tr>
									<tr>
										<td class="text-center" sdval="27">27</td>
										<td>Laryngoscope</td>
									</tr>
									<tr>
										<td class="text-center" sdval="28">28</td>
										<td>Mat</td>
									</tr>
									<tr>
										<td class="text-center" sdval="29">29</td>
										<td>Measuring Cup</td>
									</tr>
									<tr>
										<td class="text-center" sdval="30">30</td>
										<td>Nail Brush</td>
									</tr>
									<tr>
										<td class="text-center" sdval="31">31</td>
										<td>Needle Holder</td>
									</tr>
									<tr>
										<td class="text-center" sdval="32">32</td>
										<td>Non Tooth Forcep</td>
									</tr>
									<tr>
										<td class="text-center" sdval="33">33</td>
										<td>Oral Thermometer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="34">34</td>
										<td>Pillow</td>
									</tr>
									<tr>
										<td class="text-center" sdval="35">35</td>
										<td>Plastic Apron</td>
									</tr>
									<tr>
										<td class="text-center" sdval="36">36</td>
										<td>Pregnancy Kit</td>
									</tr>
									<tr>
										<td class="text-center" sdval="37">37</td>
										<td>Rubber Hot Water Bag</td>
									</tr>
									<tr>
										<td class="text-center" sdval="38">38</td>
										<td>Savlon Solution</td>
									</tr>
									<tr>
										<td class="text-center" sdval="39">39</td>
										<td>Small Bowl</td>
									</tr>
									<tr>
										<td class="text-center" sdval="40">40</td>
										<td>Small Drum</td>
									</tr>
									<tr>
										<td class="text-center" sdval="41">41</td>
										<td>Soap</td>
									</tr>
									<tr>
										<td class="text-center" sdval="42">42</td>
										<td>Soap Disk</td>
									</tr>
									<tr>
										<td class="text-center" sdval="43">43</td>
										<td>Soap Disk</td>
									</tr>
									<tr>
										<td class="text-center" sdval="44">44</td>
										<td>Spirit Lamp</td>
									</tr>
									<tr>
										<td class="text-center" sdval="45">45</td>
										<td>Spymanometer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="46">46</td>
										<td>Steam Inhaler</td>
									</tr>
									<tr>
										<td class="text-center" sdval="47">47</td>
										<td>Stetoscope</td>
									</tr>
									<tr>
										<td class="text-center" sdval="48">48</td>
										<td>Stock Register</td>
									</tr>
									<tr>
										<td class="text-center" sdval="49">49</td>
										<td>Straight Artery Forcep Big</td>
									</tr>
									<tr>
										<td class="text-center" sdval="50">50</td>
										<td>Suture Cutting Scissors</td>
									</tr>
									<tr>
										<td class="text-center" sdval="51">51</td>
										<td>Suture Needle</td>
									</tr>
									<tr>
										<td class="text-center" sdval="52">52</td>
										<td>Test Tube</td>
									</tr>
									<tr>
										<td class="text-center" sdval="53">53</td>
										<td>Test Tube Stand</td>
									</tr>
									<tr>
										<td class="text-center" sdval="54">54</td>
										<td>Toothed Thumb Forcep</td>
									</tr>
									<tr>
										<td class="text-center" sdval="55">55</td>
										<td>Torch</td>
									</tr>
									<tr>
										<td class="text-center" sdval="56">56</td>
										<td>Towel</td>
									</tr>
									<tr>
										<td class="text-center" sdval="57">57</td>
										<td>Tub</td>
									</tr>
									<tr>
										<td class="text-center" sdval="58">58</td>
										<td>Wall Clock</td>
									</tr>
									<tr>
										<td class="text-center" sdval="59">59</td>
										<td>Weighing Machine</td>
									</tr>
									<tr>
										<td class="text-center" sdval="60">60</td>
										<td>White Board</td>
									</tr>
									<tr>
										<td class="text-center" sdval="61">61</td>
										<td>Bandages</td>
									</tr>
									<tr>
										<td class="text-center" sdval="62">62</td>
										<td>Test Tube Holder</td>
									</tr>
									<tr>
										<td class="text-center" sdval="63">63</td>
										<td>Nail Cutter</td>
									</tr>
									<tr>
										<td class="text-center" sdval="64">64</td>
										<td>First Aid Kit</td>
									</tr>
									<tr>
										<td class="text-center" sdval="65">65</td>
										<td>Almirah</td>
									</tr>
									<tr>
										<td class="text-center" sdval="66">66</td>
										<td>Models</td>
									</tr>
									<tr>
										<td class="text-center" sdval="67">67</td>
										<td>Charts</td>
									</tr>
								</tbody>
							</table>
						</div>
						<div class="col-md-6">
							<table class="table table-bordered table-striped">
								<thead>
									<tr>
										<th class="text-center">Sl. No.</th>
										<th>Name of Equipments</th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td colspan="2" style="text-align:center"><strong>NUTRTION LAB</strong></td>
									</tr>
									<tr>
										<td class="text-center" sdval="1">1</td>
										<td>Refrigerator</td>
									</tr>
									<tr>
										<td class="text-center" sdval="2">2</td>
										<td>Microwave</td>
									</tr>
									<tr>
										<td class="text-center" sdval="5">5</td>
										<td>Mixer Grinder</td>
									</tr>
									<tr>
										<td class="text-center" sdval="6">6</td>
										<td>Juicer</td>
									</tr>
									<tr>
										<td class="text-center" sdval="7">7</td>
										<td>Electronic weight machine</td>
									</tr>
									<tr>
										<td class="text-center" sdval="8">8</td>
										<td>Jug</td>
									</tr>
									<tr>
										<td class="text-center" sdval="9">9</td>
										<td> Box </td>
									</tr>
									<tr>
										<td class="text-center" sdval="10">10</td>
										<td>Box</td>
									</tr>
									<tr>
										<td class="text-center" sdval="11">11</td>
										<td> Box </td>
									</tr>
									<tr>
										<td class="text-center" sdval="12">12</td>
										<td>Plate </td>
									</tr>
									<tr>
										<td class="text-center" sdval="13">13</td>
										<td>Plate </td>
									</tr>
									<tr>
										<td class="text-center" sdval="15">15</td>
										<td>Patila </td>
									</tr>
									<tr>
										<td class="text-center" sdval="16">16</td>
										<td>Patila </td>
									</tr>
									<tr>
										<td class="text-center" sdval="17">17</td>
										<td>Bowl </td>
									</tr>
									<tr>
										<td class="text-center" sdval="18">18</td>
										<td>Bowl </td>
									</tr>
									<tr>
										<td class="text-center" sdval="19">19</td>
										<td>Bowl </td>
									</tr>
									<tr>
										<td class="text-center" sdval="20">20</td>
										<td>Spoon</td>
									</tr>
									<tr>
										<td class="text-center" sdval="21">21</td>
										<td>Spoon</td>
									</tr>
									<tr>
										<td class="text-center" sdval="22">22</td>
										<td>Pan holder</td>
									</tr>
									<tr>
										<td class="text-center" sdval="24">24</td>
										<td>Spoon</td>
									</tr>
									<tr>
										<td class="text-center" sdval="25">25</td>
										<td>Fork</td>
									</tr>
									<tr>
										<td class="text-center" sdval="26">26</td>
										<td>Calchee Big filter</td>
									</tr>
									<tr>
										<td class="text-center" sdval="27">27</td>
										<td> Plate </td>
									</tr>
									<tr>
										<td class="text-center" sdval="28">28</td>
										<td> Pan</td>
									</tr>
									<tr>
										<td class="text-center" sdval="29">29</td>
										<td> Patila</td>
									</tr>
									<tr>
										<td class="text-center" sdval="30">30</td>
										<td>Pan </td>
									</tr>
									<tr>
										<td class="text-center" sdval="31">31</td>
										<td>Pan </td>
									</tr>
									<tr>
										<td class="text-center" sdval="32">32</td>
										<td>Pressure cooker</td>
									</tr>
									<tr>
										<td class="text-center" sdval="33">33</td>
										<td>Pressure cooker</td>
									</tr>
									<tr>
										<td class="text-center" sdval="34">34</td>
										<td>Pressure cooker</td>
									</tr>
									<tr>
										<td class="text-center" sdval="35">35</td>
										<td>Plain patla</td>
									</tr>
									<tr>
										<td class="text-center" sdval="36">36</td>
										<td>Lid </td>
									</tr>
									<tr>
										<td class="text-center" sdval="37">37</td>
										<td>Lid</td>
									</tr>
									<tr>
										<td class="text-center" sdval="38">38</td>
										<td>Lid</td>
									</tr>
									<tr>
										<td class="text-center" sdval="39">39</td>
										<td>Liter pan</td>
									</tr>
									<tr>
										<td class="text-center" sdval="40">40</td>
										<td> Patila</td>
									</tr>
									<tr>
										<td class="text-center" sdval="41">41</td>
										<td>Plastic glass </td>
									</tr>
									<tr>
										<td class="text-center" sdval="42">42</td>
										<td>Box </td>
									</tr>
									<tr>
										<td class="text-center" sdval="43">43</td>
										<td> Box</td>
									</tr>
									<tr>
										<td class="text-center" sdval="44">44</td>
										<td>Plastic Plate </td>
									</tr>
									<tr>
										<td class="text-center" sdval="45">45</td>
										<td>Tea filter</td>
									</tr>
									<tr>
										<td class="text-center" sdval="46">46</td>
										<td>Napkin stand</td>
									</tr>
									<tr>
										<td class="text-center" sdval="47">47</td>
										<td> Cup kit</td>
									</tr>
									<tr>
										<td class="text-center" sdval="48">48</td>
										<td>First aid box</td>
									</tr>
									<tr>
										<td class="text-center" sdval="49">49</td>
										<td>Plate</td>
									</tr>
									<tr>
										<td class="text-center" sdval="50">50</td>
										<td>dustbin </td>
									</tr>
									<tr>
										<td class="text-center" sdval="51">51</td>
										<td>Dustbin</td>
									</tr>
									<tr>
										<td class="text-center" sdval="52">52</td>
										<td>Hand mixi</td>
									</tr>
									<tr>
										<td class="text-center" sdval="53">53</td>
										<td>Tray </td>
									</tr>
									<tr>
										<td class="text-center" sdval="54">54</td>
										<td>Freezer plate</td>
									</tr>
									<tr>
										<td class="text-center" sdval="55">55</td>
										<td>Weighing and Measuring Balance </td>
									</tr>
									<tr>
										<td class="text-center" sdval="56">56</td>
										<td>Bowl </td>
									</tr>
									<tr>
										<td class="text-center" sdval="57">57</td>
										<td>Chapati Roller</td>
									</tr>
									<tr>
										<td class="text-center" sdval="58">58</td>
										<td>Cutting board</td>
									</tr>
									<tr>
										<td class="text-center" sdval="59">59</td>
										<td>Flower Pot </td>
									</tr>
									<tr>
										<td class="text-center" sdval="60">60</td>
										<td>Rolling board</td>
									</tr>
									<tr>
										<td class="text-center" sdval="61">61</td>
										<td>Flannel board</td>
									</tr>
									<tr>
										<td class="text-center" sdval="62">62</td>
										<td>Wood counter</td>
									</tr>
									<tr>
										<td class="text-center" sdval="63">63</td>
										<td>Green Board</td>
									</tr>
									<tr>
										<td class="text-center" sdval="64">64</td>
										<td>kitchen Shelf</td>
									</tr>
									<tr>
										<td class="text-center" sdval="65">65</td>
										<td>Enamel cups </td>
									</tr>
									<tr>
										<td class="text-center" sdval="66">66</td>
										<td>Enamel bowl </td>
									</tr>
									<tr>
										<td class="text-center" sdval="67">67</td>
										<td>Tea pan</td>
									</tr>
									<tr>
										<td class="text-center" sdval="68">68</td>
										<td>Enamel plate </td>
									</tr>
									<tr>
										<td class="text-center" sdval="69">69</td>
										<td>Bed sheet </td>
									</tr>
									<tr>
										<td class="text-center" sdval="70">70</td>
										<td>Sheet </td>
									</tr>
									<tr>
										<td class="text-center" sdval="71">71</td>
										<td>kichen apron</td>
									</tr>
									<tr>
										<td class="text-center" sdval="72">72</td>
										<td>Towel </td>
									</tr>
									<tr>
										<td class="text-center" sdval="73">73</td>
										<td>Gas pipeline</td>
									</tr>
									<tr>
										<td class="text-center" sdval="74">74</td>
										<td>Regulator with pipe</td>
									</tr>
									<tr>
										<td class="text-center" sdval="75">75</td>
										<td>Gas cylinder</td>
									</tr>
									<tr>
										<td class="text-center" sdval="76">76</td>
										<td>Gas stove</td>
									</tr>
									<tr>
										<td class="text-center" sdval="77">77</td>
										<td>Almirah</td>
									</tr>
									<tr>
										<td class="text-center" sdval="78">78</td>
										<td>knife</td>
									</tr>
									<tr>
										<td class="text-center" sdval="79">79</td>
										<td>weight</td>
									</tr>
									<tr>
										<td class="text-center" sdval="80">80</td>
										<td>Taraju</td>
									</tr>
									<tr>
										<td class="text-center" sdval="81">81</td>
										<td>Tongs</td>
									</tr>
									<tr>
										<td class="text-center" sdval="82">82</td>
										<td>Hand wash</td>
									</tr>
									<tr>
										<td class="text-center" sdval="83">83</td>
										<td>Table mat</td>
									</tr>
								</tbody>
							</table>
						</div>
					</div>
				</div>
			</div>
		</section>
	</div>
</div>
</div>
<!-- Bootstrap JS and dependencies -->

@endsection