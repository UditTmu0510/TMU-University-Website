@extends('layouts.university.colleges.nursing_with_sidebar')
@section('content')



<div class="main-content">
    <div class="container">
        <h1 class="tmu-text-primary tmu-page-heading"><span> IJIH Current</span><span> Issue</span></h1>
        <div class="container">
            <div class="row mb-3">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th style="white-space: nowrap; text-align: center; width: 10%;">Sl. No</th>
                                <th>Content</th>
                                <th style="white-space: nowrap; text-align: center; width: 15%;">Page No</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td style="text-align: center;">1.</td>
                                <td>
                                    <a href="{{asset('/pdf/journals/nursing/archives/1-min.pdf')}}" target="_blank">
                                        How Nursing Theory Informs Implementation Science: A Systematic Review of Evidence-Based Practices and Nurse-Led Initiatives for Sustainable Healthcare
                                    </a>
                                </td>
                                <td style="text-align: center;">1 to 21</td>
                            </tr>
                            <tr>
                                <td style="text-align: center;">2.</td>
                                <td>
                                    <a href="{{asset('/pdf/journals/nursing/archives/2-min.pdf')}}" target="_blank">
                                        Evaluating the Impact of Video-Assisted Teaching on Personal Hygiene Knowledge Among School Children in Moradabad, Uttar Pradesh
                                    </a>
                                </td>
                                <td style="text-align: center;">22 to 30</td>
                            </tr>
                            <tr>
                                <td style="text-align: center;">3.</td>
                                <td>
                                    <a href="{{asset('/pdf/journals/nursing/archives/3-min.pdf')}}" target="_blank">
                                        Understanding Disaster Preparedness: A Knowledge Assessment Among Agriculture Students At Teerthanker Mahaveer University, Moradabad, U.P.
                                    </a>
                                </td>
                                <td style="text-align: center;">31 to 39</td>
                            </tr>
                            <tr>
                                <td style="text-align: center;">4.</td>
                                <td>
                                    <a href="{{asset('/pdf/journals/nursing/archives/4-min.pdf')}}" target="_blank">
                                        Point Prevalence of Musculoskeletal Pain Among Bankers Visiting Teerthanker Mahaveer University OPD
                                    </a>
                                </td>
                                <td style="text-align: center;">40 to 45</td>
                            </tr>
                            <tr>
                                <td style="text-align: center;">5.</td>
                                <td>
                                    <a href="{{asset('/pdf/journals/nursing/archives/5-min.pdf')}}" target="_blank">
                                        Patients’ Satisfaction Regarding Nurse-Patient Communication Attending a Teaching Hospital, Bharatpur, Chitwan
                                    </a>
                                </td>
                                <td style="text-align: center;">46 to 52</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- Bootstrap JS and dependencies -->
</div>


@endsection