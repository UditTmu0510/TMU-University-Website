@extends('layouts.university.colleges.paramedical_with_sidebar')
@section('content')
    <div class="main-content">


        <h1 class="tmu-text-primary tmu-page-heading"><span>Anti Ragging </span><span> Committee</span></h1>

        <div class="container">
            <div class="row mb-3">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th class="text-center">Sl. No.</th>
                                <th class="text-center">Name</th>
                                <th class="text-center">Designation</th>
                                <th class="text-center">Contact No.</th>
                                <th class="text-center">Email</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center">1.</td>
                                <td class="text-center">(Prof.) Dr. Navneet Kumar</td>
                                <td class="text-center">Chair Person</td>
                                <td class="text-center">9997378899</td>
                                <td class="text-center">Principal.paramedical@tmu.ac.in</td>
                            </tr>
                            <tr>
                                <td class="text-center">2.</td>
                                <td class="text-center">Mr. Baijnath Das</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">8505866824</td>
                                <td class="text-center">Baijnath.paramedical@tmu.ac.in</td>
                            </tr>
                            <tr>
                                <td class="text-center">3.</td>
                                <td class="text-center">Mr. Akash Chauhan</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">8191980081</td>
                                <td class="text-center">Akashc.paramedical@tmu.ac.in</td>
                            </tr>
                            <tr>
                                <td class="text-center">4.</td>
                                <td class="text-center">Ms. Anjali Rani</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">6398029319</td>
                                <td class="text-center">Anjalir.paramedical@tmu.ac.in</td>
                            </tr>
                            <tr>
                                <td class="text-center">5.</td>
                                <td class="text-center">Mr. Amit Bisht</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">7088865263</td>
                                <td class="text-center">Amitb.paramedical@tmu.ac.in</td>
                            </tr>

                        </tbody>
                    </table>
                </div>
            </div>

            <div class="container">
                <div class="row justify-content-center mb-5 mt-5">
                    <div class="col-md-8 mb-3 mb-sm-0 ps-3 pe-3" style="background-color: #001055;">
                        <div class="card border-light my-3">
                            <div class="card-body text-center">
                                <h4 class="card-title tmu-text-primary display-6"><span>TOLL FREE ANTI RAGGING </span></h4>
                                <p class="card-text"><span class="highlight">24X7 HELPLINE NO. 1800-180-5522 </span></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="row mb-3">
                <h2 class="tmu-text-primary tmu-page-heading"><span> Proctorial </span><span>Board</span></h2>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th class="text-center">Sl. No. </th>
                                <th class="text-center">Name</th>
                                <th class="text-center">Designation</th>
                                <th class="text-center">Contact No.</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center">1.</td>
                                <td class="text-center">Mr. Ravi Kumar</td>
                                <td class="text-center">Proctor</td>
                                <td class="text-center">9690444424</td>
                            </tr>

                            <tr>
                                <td class="text-center">2.</td>
                                <td class="text-center">Ms Priyanka Singh</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">9536150439</td>
                            </tr>

                            <tr>
                                <td class="text-center">3.</td>
                                <td class="text-center">Mr. Rakesh Kumar Yadav</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">7387184265</td>
                            </tr>

                            <tr>
                                <td class="text-center">4.</td>
                                <td class="text-center">Mr. Shivam Agrawal</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">7906961867</td>
                            </tr>

                            <tr>
                                <td class="text-center">5.</td>
                                <td class="text-center">Mr Arvind Kumar</td>
                                <td class="text-center">Member</td>
                                <td class="text-center">9458706894</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    </div>
    <!-- Bootstrap JS and dependencies -->
@endsection
