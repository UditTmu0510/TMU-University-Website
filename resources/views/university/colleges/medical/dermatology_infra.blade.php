@extends('layouts.university.colleges.medical_with_sidebar')
@section('content')

<div class="main-content">
    <div class="container">

        <section class="about-section bg-light mt-4">
            <div class="container">
                <div class="text-content p-4">
                    <h1 class="tmu-text-primary tmu-page-heading pt-md-5"><span> Dermatology Geo</span><span> Tagged Infrastructure </span></h1>
                </div>
                <div class="container p-3">
                    <h3 class="tmu-text-primary"><span>Library</span><span></span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/library1_7_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/library1_7_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/library2_8_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/library2_8_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>HOD </span><span> Room</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/hod_room1_3_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/hod_room1_3_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/hod_room2_4_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/hod_room2_4_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>IPD </span><span>I</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/IPD_I_1_5_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/IPD_I_1_5_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/IPD_I_2_6_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/IPD_I_2_6_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>OPD </span><span> I</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_I_1_9_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_I_1_9_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_I_2_10_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_I_2_10_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>OPD </span><span> II</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_II_1_11_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_II_1_11_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_II_2_12_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_II_2_12_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>OPD </span><span> III</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_III_1_13_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_III_1_13_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_III_2_14_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_III_2_14_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>OPD </span><span> IV</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_1_15_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_1_15_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_2_16_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_2_16_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_3_17_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/opd_IV_3_17_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Demonstration </span><span> Room</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/demonstration_room_I_1_1_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/demonstration_room_I_1_1_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/dermatology/demonstration_room_I_2_2_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/dermatology/demonstration_room_I_2_2_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <tbody>
                            <tr>
                                <td colspan="2">LIST OF EQUIPMENTS</td>
                            </tr>
                            <tr>
                                <td>Sr No.</td>
                                <td>Name of Equipment</td>
                            </tr>
                            <tr>
                                <td>1</td>
                                <td>Facilities for examining smears for bacteria, Fungi, mycobacteria and acantholytic cells</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(a) Light microscope, Electro cautry machine</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(b) Gram&rsquo;s stain</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(c) Zeihl-Neelsen&rsquo;s stain</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(d)10-20% KOH for fungus identification</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(e) Giemsa stain</td>
                            </tr>
                            <tr>
                                <td>2</td>
                                <td>Facilities for electrosurgery and chemosurgery</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(a) Electro-cautery machine</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(b) Trichloracetic acid (25%, 35% and 70%)</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(c)Glycolic acid&nbsp; (10%, 35%, 50% and 70%)</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(d)Jessner's Peel&nbsp;</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(e)Salicylic acid (10%, 20% and 30% )</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>(f)Phenol 66%</td>
                            </tr>
                            <tr>
                                <td>3</td>
                                <td>acne comedone extractor</td>
                            </tr>
                            <tr>
                                <td>4</td>
                                <td>Wood's Lamp</td>
                            </tr>
                            <tr>
                                <td>5</td>
                                <td>CO2 LASER</td>
                            </tr>
                            <tr>
                                <td>6</td>
                                <td>ND Yag LASER&nbsp;</td>
                            </tr>
                            <tr>
                                <td>7</td>
                                <td>Q SWITCHED LASER</td>
                            </tr>
                            <tr>
                                <td>8</td>
                                <td>DERMABRADER</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>CENTRIFUGE MACHINE</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </section>
    </div>
</div>
</div>

@endsection