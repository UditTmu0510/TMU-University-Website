@extends('layouts.university.colleges.medical_with_sidebar')
@section('content')


<div class="main-content">
	<div class="container">
		<h1 class="tmu-text-primary tmu-page-heading"><span>Financial Year </span><span> 2024-25 </span></h1>
		<div class="table-responsive ps-4 pe-4">
			<table class="table table-bordered table-striped">
				<thead>
					<th class="text-center">Sl. No.</th>
					<th class="text-center">Categeory</th>
					<th class="text-center">State Govt Stipend</th>
					<th class="text-center">College's Stipend</th>
					<th class="text-center">April</th>
					<th class="text-center">May</th>
					<th class="text-center">June</th>
					<th class="text-center">July</th>
					<th class="text-center">August</th>
					<th class="text-center">September</th>
					<th class="text-center">October</th>
					<th class="text-center">November</th>
					<th class="text-center">December</th>
					<th class="text-center">January</th>
					<th class="text-center">February</th>
					<th class="text-center">March</th>
				</thead>
				<tbody>
					<tr>
						<td class="text-center">1.</td>
						<td class="text-center">Interns (MBBS)</td>
						<td class="text-center">12000</td>
						<td class="text-center">6000</td>
						<td class="text-center">165</td>
						<td class="text-center">165</td>
						<td class="text-center">171</td>
						<td class="text-center">176</td>
						<td class="text-center">152</td>
						<td class="text-center">152</td>
						<td class="text-center">152</td>
						<td class="text-center">152</td>
						<td class="text-center">152</td>
						<td class="text-center"></td>
						<td class="text-center"></td>
						<td class="text-center"></td>
					
					</tr>
					<tr>
						<td colspan="16" style="text-align:left;">Post-Graduate Residents</td>
					</tr>
					<tr>
						<td class="text-center">2.</td>
						<td class="text-center">Ist Year (MD/MS)</td>
						<td class="text-center">15600 - 39100</td>
						<td class="text-center">69600</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center">119</td>
						<td class="text-center"></td>
						<td class="text-center"></td>
						<td class="text-center"></td>

					</tr>
					<tr>
						<td class="text-center">3.</td>
						<td class="text-center">IInd Year (MD/MS)</td>
						<td class="text-center">15600 – 39100</td>
						<td class="text-center">71680</td>
						<td class="text-center">107</td>
						<td class="text-center">107</td>
						<td class="text-center">107</td>
						<td class="text-center">107</td>
						<td class="text-center">106</td>
						<td class="text-center">106</td>
						<td class="text-center">106</td>
						<td class="text-center">106</td>
						<td class="text-center">106</td>
						<td class="text-center"></td>
						<td class="text-center"></td>
						<td class="text-center"></td>
					</tr>
					<tr>
						<td class="text-center">4.</td>
						<td class="text-center">IIIrd Year (MD/MS)</td>
						<td class="text-center">15600 – 39100</td>
						<td class="text-center">73824</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center">111</td>
						<td class="text-center"></td>
						<td class="text-center"></td>
						<td class="text-center"></td>
					</tr>
					<tr>
						<td colspan="16" style="text-align:left;">Senior Residents or PGs in Super Specialty:</td>
					</tr>
					<tr>
						<td>5.</td>
						<td>Ist year (DM/MCh)</td>
						<td colspan="14" style="text-align:center;">N/A</td>

						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->

					</tr>
					<tr>
						<td>6.</td>
						<td>IInd Year (DM/MCh)</td>
						<td colspan="14" style="text-align:center;">N/A</td>
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
					</tr>
					<tr>
						<td>7.</td>
						<td>IIIrd Year(DM/MCh)</td>
						<td colspan="14" style="text-align:center;">N/A</td>
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
						<!--<td></td>-->
					</tr>

				</tbody>
			</table>
		</div>
	</div>
</div>
</div>

@endsection