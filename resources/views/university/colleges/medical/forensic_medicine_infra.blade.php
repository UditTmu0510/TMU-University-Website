@extends('layouts.university.colleges.medical_with_sidebar')
@section('content')

<div class="main-content">
    <div class="container">

        <section class="about-section bg-light mt-4">
            <div class="container">
                <div class="text-content p-4">
                    <h1 class="tmu-text-primary tmu-page-heading pt-md-5"><span> Forensic Medicine Geo</span><span> Tagged Infrastructure </span></h1>
                </div>
                <div class="container p-3">

                    <h3 class="tmu-text-primary"><span>Mortuary </span><span> Block</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block1_9_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block1_9_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block2_10_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block2_10_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block3_11_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/mortuary_block3_11_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Research </span><span> Lab</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/research_lab1_15_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/research_lab1_15_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/research_lab2_16_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/research_lab2_16_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Forensic </span><span> Library</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library1_5_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library1_5_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library2_6_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library2_6_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Demonstration </span><span> Room 1</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_I_1_1_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_I_1_1_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_I_2_2_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_I_2_2_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Demonstration </span><span> Room 2</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_II_1_3_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_II_1_3_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_II_2_4_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/demonstration_room_II_2_4_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>HOD </span><span> Room</span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/hod_room1_7_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/hod_room1_7_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/hod_room2_8_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/hod_room2_8_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Library </span><span></span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library1_5_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library1_5_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library2_6_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/forensic_library2_6_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>
                    <h3 class="tmu-text-primary"><span>Museum </span><span></span></h3>
                    <div class="masonry-thumbs grid-container row row-cols-4 mb-3" data-lightbox="gallery">
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum1_12_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum1_12_11zon.jpg')}}" alt="Gallery Thumb 2"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum2_13_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum2_13_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                        <a class="grid-item" href="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum3_14_11zon.jpg')}}" data-lightbox="gallery-item"><img src="{{asset('/assets/img/college_departments/medical/forensicmedicine/museum3_14_11zon.jpg')}}" alt="Gallery Thumb 3"></a>
                    </div>
                    </br>


                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <tbody>
                            <tr>
                                <td colspan="4">
                                    <center><b>LIST OF WEAPONS IN FORENSIC MEDICINE DEPARTMENT</b></center>
                                </td>
                            </tr>
                            <tr>
                                <td>S.NO.</td>
                                <td>Name of weapon</td>
                                <td>S.NO.</td>
                                <td>Name of weapon</td>
                            </tr>
                            <tr>
                                <td>1.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Scissors (Garden)</td>
                                <td>51</td>
                                <td>Air pistol</td>
                            </tr>
                            <tr>
                                <td>2.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>HATHRASHI KNIFE</td>
                                <td>52</td>
                                <td>Probe</td>
                            </tr>
                            <tr>
                                <td>3.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Hansia</td>
                                <td>53</td>
                                <td>Hathkari</td>
                            </tr>
                            <tr>
                                <td>4.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Lathi</td>
                                <td>54</td>
                                <td>Metal cutter</td>
                            </tr>
                            <tr>
                                <td>5.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Iron Rod Twisted</td>
                                <td>55</td>
                                <td>Iron Plate</td>
                            </tr>
                            <tr>
                                <td>6.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Farsa</td>
                                <td>56</td>
                                <td>Ball</td>
                            </tr>
                            <tr>
                                <td>7.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Kulhadi (Axe)</td>
                                <td>57</td>
                                <td>Kantedar Tar</td>
                            </tr>
                            <tr>
                                <td>8.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Air Gun</td>
                                <td>58</td>
                                <td>Pistol model</td>
                            </tr>
                            <tr>
                                <td>9.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>Bat</td>
                                <td>59</td>
                                <td>Nail plucker</td>
                            </tr>
                            <tr>
                                <td>10.&nbsp;&nbsp; &nbsp;</td>
                                <td>Hammer</td>
                                <td>60</td>
                                <td>STONE</td>
                            </tr>
                            <tr>
                                <td>11.&nbsp;&nbsp; &nbsp;</td>
                                <td>Whip EKKA</td>
                                <td>61</td>
                                <td>Broken Glass Pane</td>
                            </tr>
                            <tr>
                                <td>12.&nbsp;&nbsp; &nbsp;</td>
                                <td>Bull worker</td>
                                <td>62</td>
                                <td>Rope (Resham)</td>
                            </tr>
                            <tr>
                                <td>13.&nbsp;&nbsp; &nbsp;</td>
                                <td>Nail</td>
                                <td>63</td>
                                <td>Paper weight</td>
                            </tr>
                            <tr>
                                <td>14.&nbsp;&nbsp; &nbsp;</td>
                                <td>Burma</td>
                                <td>64</td>
                                <td>Dog chain</td>
                            </tr>
                            <tr>
                                <td>15.&nbsp;&nbsp; &nbsp;</td>
                                <td>Needle</td>
                                <td>65</td>
                                <td>Vegetable knife</td>
                            </tr>
                            <tr>
                                <td>16.&nbsp;&nbsp; &nbsp;</td>
                                <td>Belcha (spade)</td>
                                <td>66</td>
                                <td>Animal chain&nbsp;</td>
                            </tr>
                            <tr>
                                <td>17.&nbsp;&nbsp; &nbsp;</td>
                                <td>Screw Driver</td>
                                <td>67</td>
                                <td>Table Fork</td>
                            </tr>
                            <tr>
                                <td>18.&nbsp;&nbsp; &nbsp;</td>
                                <td>Wood cutter</td>
                                <td>68</td>
                                <td>Blade</td>
                            </tr>
                            <tr>
                                <td>19.&nbsp;&nbsp; &nbsp;</td>
                                <td>A METAL BALL WITH CHAIN&nbsp;</td>
                                <td>69</td>
                                <td>Parkhee</td>
                            </tr>
                            <tr>
                                <td>20.&nbsp;&nbsp; &nbsp;</td>
                                <td>Vasuli (Mason)</td>
                                <td>70</td>
                                <td>Sword&nbsp;</td>
                            </tr>
                            <tr>
                                <td>21.&nbsp;&nbsp; &nbsp;</td>
                                <td>Sabbal</td>
                                <td>71</td>
                                <td>Paper knife</td>
                            </tr>
                            <tr>
                                <td>22.&nbsp;&nbsp; &nbsp;</td>
                                <td>Ballam</td>
                                <td>72</td>
                                <td>Kada</td>
                            </tr>
                            <tr>
                                <td>23.&nbsp;&nbsp; &nbsp;</td>
                                <td>Gainty</td>
                                <td>73</td>
                                <td>Bird shot</td>
                            </tr>
                            <tr>
                                <td>24.&nbsp;&nbsp; &nbsp;</td>
                                <td>Gandaasa</td>
                                <td>74</td>
                                <td>Butter knife</td>
                            </tr>
                            <tr>
                                <td>25.&nbsp;&nbsp; &nbsp;</td>
                                <td>Belan</td>
                                <td>75</td>
                                <td>Wrench</td>
                            </tr>
                            <tr>
                                <td>26.&nbsp;&nbsp; &nbsp;</td>
                                <td>Kanni</td>
                                <td>76</td>
                                <td>Scissor (Barber)</td>
                            </tr>
                            <tr>
                                <td>27.&nbsp;&nbsp; &nbsp;</td>
                                <td>Electric Wire</td>
                                <td>77</td>
                                <td>Rope (jute)</td>
                            </tr>
                            <tr>
                                <td>28.&nbsp;&nbsp; &nbsp;</td>
                                <td>Hockey stick</td>
                                <td>78</td>
                                <td>Churra</td>
                            </tr>
                            <tr>
                                <td>29.&nbsp;&nbsp; &nbsp;</td>
                                <td>GOLTA</td>
                                <td>79</td>
                                <td>Razor</td>
                            </tr>
                            <tr>
                                <td>30.&nbsp;&nbsp; &nbsp;</td>
                                <td>Seek Salai</td>
                                <td>80</td>
                                <td>Cycle chain</td>
                            </tr>
                            <tr>
                                <td>31.&nbsp;&nbsp; &nbsp;</td>
                                <td>Daranti</td>
                                <td>81</td>
                                <td>HOOK</td>
                            </tr>
                            <tr>
                                <td>32.&nbsp;&nbsp; &nbsp;</td>
                                <td>Trishul</td>
                                <td>82</td>
                                <td>Revolver</td>
                            </tr>
                            <tr>
                                <td>33.&nbsp;&nbsp; &nbsp;</td>
                                <td>Mungri</td>
                                <td>83</td>
                                <td>KUDAL</td>
                            </tr>
                            <tr>
                                <td>34.&nbsp;&nbsp; &nbsp;</td>
                                <td>Musli</td>
                                <td>84</td>
                                <td>Gullel</td>
                            </tr>
                            <tr>
                                <td>35.&nbsp;&nbsp; &nbsp;</td>
                                <td>Police Rool</td>
                                <td>85</td>
                                <td>Iron weight</td>
                            </tr>
                            <tr>
                                <td>36.&nbsp;&nbsp; &nbsp;</td>
                                <td>Gandasa</td>
                                <td>86</td>
                                <td>Rope (nylon)</td>
                            </tr>
                            <tr>
                                <td>37.&nbsp;&nbsp; &nbsp;</td>
                                <td>Khurpi</td>
                                <td>87</td>
                                <td>Patasi</td>
                            </tr>
                            <tr>
                                <td>38.&nbsp;&nbsp; &nbsp;</td>
                                <td>Gandasi</td>
                                <td>88</td>
                                <td>Dinner knife</td>
                            </tr>
                            <tr>
                                <td>39.&nbsp;&nbsp; &nbsp;</td>
                                <td>Meat chopper</td>
                                <td>89</td>
                                <td>Tyre lever</td>
                            </tr>
                            <tr>
                                <td>40.&nbsp;&nbsp; &nbsp;</td>
                                <td>Favra</td>
                                <td>90</td>
                                <td>ICE PICK</td>
                            </tr>
                            <tr>
                                <td>41.&nbsp;&nbsp; &nbsp;</td>
                                <td>Cloth meter</td>
                                <td>91</td>
                                <td>Chissel</td>
                            </tr>
                            <tr>
                                <td>42.&nbsp;&nbsp; &nbsp;</td>
                                <td>Police Cane</td>
                                <td>92</td>
                                <td>Broken bottle</td>
                            </tr>
                            <tr>
                                <td>43.&nbsp;&nbsp; &nbsp;</td>
                                <td>Wicket</td>
                                <td>93</td>
                                <td>Sauce bottle&nbsp;</td>
                            </tr>
                            <tr>
                                <td>44.&nbsp;&nbsp; &nbsp;</td>
                                <td>Bhala</td>
                                <td>94</td>
                                <td>Nanchaku</td>
                            </tr>
                            <tr>
                                <td>45.&nbsp;&nbsp; &nbsp;</td>
                                <td>Chimta</td>
                                <td>95</td>
                                <td>Hunter</td>
                            </tr>
                            <tr>
                                <td>46.&nbsp;&nbsp; &nbsp;</td>
                                <td>CHOPPER</td>
                                <td>96</td>
                                <td>Gun cartridge(Shot Gun)Empty</td>
                            </tr>
                            <tr>
                                <td>47.&nbsp;&nbsp; &nbsp;</td>
                                <td>BASE BALL BAT</td>
                                <td>97</td>
                                <td>Gun Cartridge (Rifle)Empty</td>
                            </tr>
                            <tr>
                                <td>48.&nbsp;&nbsp; &nbsp;</td>
                                <td>Petha Pinner</td>
                                <td>98</td>
                                <td>Gun Cartridge (un used) Rifle</td>
                            </tr>
                            <tr>
                                <td>49.&nbsp;&nbsp; &nbsp;</td>
                                <td>Ice cutter</td>
                                <td>99</td>
                                <td>SANDASI</td>
                            </tr>
                            <tr>
                                <td>50.&nbsp;&nbsp; &nbsp;</td>
                                <td>Scissor (Tailor)</td>
                                <td>100</td>
                                <td>PUNJA</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <br /><br />
                <div class="table-responsive">
                    <table class="table table-bordered table-striped ">
                        <tbody>
                            <tr>
                                <td colspan="3">
                                    <center><b>LIST OF POISON IN FORENSIC MEDICINE DEPARTMENT</b></center>
                                </td>
                            </tr>
                            <tr>
                                <td>S.No.</td>
                                <td>Type of Poison</td>
                                <td>Name of Poison</td>
                            </tr>
                            <tr>
                                <td>1. </td>
                                <td>Irritant poison</td>
                                <td>Karihari</td>
                            </tr>
                            <tr>
                                <td>2. </td>
                                <td>Irritant poison</td>
                                <td>Abrus precatorius (Red)</td>
                            </tr>
                            <tr>
                                <td>3. </td>
                                <td>Irritant poison</td>
                                <td>Abrus precatorius (white)</td>
                            </tr>
                            <tr>
                                <td>4. </td>
                                <td>Irritant poison</td>
                                <td>Argemona Mexicana(Satyanashi/pila dhatura)</td>
                            </tr>
                            <tr>
                                <td>5. </td>
                                <td>Irritant poison</td>
                                <td>Citrulus colocynthis(Bitter apple)</td>
                            </tr>
                            <tr>
                                <td>6. </td>
                                <td>Irritant poison</td>
                                <td>Croton trigilum (Jamalghota)Croton Tiglium)</td>
                            </tr>
                            <tr>
                                <td>7. </td>
                                <td>Irritant poison</td>
                                <td>Ricinus communis (Castor/Arandi)</td>
                            </tr>
                            <tr>
                                <td>8. </td>
                                <td>Irritant poison</td>
                                <td>Semicarpus anacardium(Marking Nut)</td>
                            </tr>
                            <tr>
                                <td>9. </td>
                                <td>Irritant poison</td>
                                <td>Ipomea hediracea</td>
                            </tr>
                            <tr>
                                <td>10. </td>
                                <td>Cerebral inebrient</td>
                                <td>Poppy ka doda (Papaver Somniferum)</td>
                            </tr>
                            <tr>
                                <td>11. </td>
                                <td>Irritant poison</td>
                                <td>Ritha</td>
                            </tr>
                            <tr>
                                <td>12. </td>
                                <td>Irritant poison</td>
                                <td>Kalikuthki</td>
                            </tr>
                            <tr>
                                <td>13. </td>
                                <td>Irritant poison</td>
                                <td>Aloe vera</td>
                            </tr>
                            <tr>
                                <td>14. </td>
                                <td>Irritant poison</td>
                                <td>Calotropis(Madar/Aak)</td>
                            </tr>
                            <tr>
                                <td>15. </td>
                                <td>Irritant poison</td>
                                <td>Kachur</td>
                            </tr>
                            <tr>
                                <td>16. </td>
                                <td>Irritant poison</td>
                                <td>Carrot Seed</td>
                            </tr>
                            <tr>
                                <td>17. </td>
                                <td>Irritant poison</td>
                                <td>Jatropha curcas (rattan jat)</td>
                            </tr>
                            <tr>
                                <td>18. </td>
                                <td>Irritant poison</td>
                                <td>Ergotoxin &ndash; Ergotamine</td>
                            </tr>
                            <tr>
                                <td>19. </td>
                                <td>Irritant poison</td>
                                <td>Lal chitra(Plumbago Rosea)</td>
                            </tr>
                            <tr>
                                <td>20. </td>
                                <td>Irritant poison</td>
                                <td>Carica papaya seeds</td>
                            </tr>
                            <tr>
                                <td>21. </td>
                                <td>Animal poison</td>
                                <td>Wasp</td>
                            </tr>
                            <tr>
                                <td>22. </td>
                                <td>Animal poison</td>
                                <td>Cobra</td>
                            </tr>
                            <tr>
                                <td>23. </td>
                                <td>Animal poison</td>
                                <td>Tree snake</td>
                            </tr>
                            <tr>
                                <td>24. </td>
                                <td>Animal poison</td>
                                <td>Centipede</td>
                            </tr>
                            <tr>
                                <td>25. </td>
                                <td>Animal poison</td>
                                <td>Scorpion</td>
                            </tr>
                            <tr>
                                <td>26. </td>
                                <td>Animal poison</td>
                                <td>Honey bee</td>
                            </tr>
                            <tr>
                                <td>27. </td>
                                <td>Animal poison</td>
                                <td>Banded krait</td>
                            </tr>
                            <tr>
                                <td>28. </td>
                                <td>Animal poison</td>
                                <td>Viper</td>
                            </tr>
                            <tr>
                                <td>29. </td>
                                <td>Animal poison</td>
                                <td>Lizard</td>
                            </tr>
                            <tr>
                                <td>30. </td>
                                <td>Animal poison</td>
                                <td>Reptiles (Sea snake)</td>
                            </tr>
                            <tr>
                                <td>31. </td>
                                <td>Irritant poison</td>
                                <td>Capsicum annum</td>
                            </tr>
                            <tr>
                                <td>32. </td>
                                <td>Irritant poison</td>
                                <td>Needle</td>
                            </tr>
                            <tr>
                                <td>33. </td>
                                <td>Irritant poison</td>
                                <td>Animal nail</td>
                            </tr>
                            <tr>
                                <td>34. </td>
                                <td>Irritant poison</td>
                                <td>Diamond</td>
                            </tr>
                            <tr>
                                <td>35. </td>
                                <td>Irritant poison</td>
                                <td>Powdered Glass</td>
                            </tr>
                            <tr>
                                <td>36. </td>
                                <td>Irritant poison</td>
                                <td>Human hair</td>
                            </tr>
                            <tr>
                                <td>37. </td>
                                <td>Irritant poison</td>
                                <td>Tincture iodine</td>
                            </tr>
                            <tr>
                                <td>38. </td>
                                <td>Irritant poison</td>
                                <td>Iodine</td>
                            </tr>
                            <tr>
                                <td>39. </td>
                                <td>Irritant poison</td>
                                <td>Dichlor Vos</td>
                            </tr>
                            <tr>
                                <td>40. </td>
                                <td>Irritant poison</td>
                                <td>Borax</td>
                            </tr>
                            <tr>
                                <td>41. </td>
                                <td>Metallic poison</td>
                                <td>Safed phitkari</td>
                            </tr>
                            <tr>
                                <td>42. </td>
                                <td>Metallic poison</td>
                                <td>Barium Carbonate</td>
                            </tr>
                            <tr>
                                <td>43. </td>
                                <td>Metallic poison</td>
                                <td>Arsenic trisulphite</td>
                            </tr>
                            <tr>
                                <td>44. </td>
                                <td>Metallic poison</td>
                                <td>Potassium Arsenite</td>
                            </tr>
                            <tr>
                                <td>45. </td>
                                <td>Metallic poison</td>
                                <td>Antimony trisulphite</td>
                            </tr>
                            <tr>
                                <td>46. </td>
                                <td>Metallic poison</td>
                                <td>Copper arsenite</td>
                            </tr>
                            <tr>
                                <td>47. </td>
                                <td>Metallic poison</td>
                                <td>Copper aceto arsenite</td>
                            </tr>
                            <tr>
                                <td>48. </td>
                                <td>Metallic poison</td>
                                <td>Mercury</td>
                            </tr>
                            <tr>
                                <td>49. </td>
                                <td>Metallic poison</td>
                                <td>Magnesium Sulphate</td>
                            </tr>
                            <tr>
                                <td>50. </td>
                                <td>Metallic poison</td>
                                <td>Mercurious chloride</td>
                            </tr>
                            <tr>
                                <td>51. </td>
                                <td>Metallic poison</td>
                                <td>Lead Monooxide</td>
                            </tr>
                            <tr>
                                <td>52. </td>
                                <td>Metallic poison</td>
                                <td>Copper sulphate</td>
                            </tr>
                            <tr>
                                <td>53. </td>
                                <td>Metallic poison</td>
                                <td>Lead Tetraoxide</td>
                            </tr>
                            <tr>
                                <td>54. </td>
                                <td>Metallic poison</td>
                                <td>Arsenic Trioxide</td>
                            </tr>
                            <tr>
                                <td>55. </td>
                                <td>Metallic poison</td>
                                <td>Mercuric sulphide</td>
                            </tr>
                            <tr>
                                <td>56. </td>
                                <td>Metallic poison</td>
                                <td>Potassium permagnate</td>
                            </tr>
                            <tr>
                                <td>57. </td>
                                <td>Metallic poison</td>
                                <td>Lead sulphide (Surma)</td>
                            </tr>
                            <tr>
                                <td>58. </td>
                                <td>Metallic poison</td>
                                <td>Arsenious oxide</td>
                            </tr>
                            <tr>
                                <td>59. </td>
                                <td>Drug</td>
                                <td>Oxazepam</td>
                            </tr>
                            <tr>
                                <td>60. </td>
                                <td>Drug</td>
                                <td>Soluble insulin</td>
                            </tr>
                            <tr>
                                <td>61. </td>
                                <td>Drug</td>
                                <td>Chloramphenicol</td>
                            </tr>
                            <tr>
                                <td>62. </td>
                                <td>corrosive</td>
                                <td>Carbolic acid</td>
                            </tr>
                            <tr>
                                <td>63. </td>
                                <td>Corrosive</td>
                                <td>Sodium hydroxide</td>
                            </tr>
                            <tr>
                                <td>64. </td>
                                <td>Corrosive</td>
                                <td>Aspirin</td>
                            </tr>
                            <tr>
                                <td>65. </td>
                                <td>Corrosive</td>
                                <td>Vinegar</td>
                            </tr>
                            <tr>
                                <td>66. </td>
                                <td>corrosive</td>
                                <td>Sulphuric acid</td>
                            </tr>
                            <tr>
                                <td>67. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Methyl alcohol</td>
                            </tr>
                            <tr>
                                <td>68. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Sulphanilamide</td>
                            </tr>
                            <tr>
                                <td>69. </td>
                                <td>Cerebral Inebriant</td>
                                <td>DDT</td>
                            </tr>
                            <tr>
                                <td>70. </td>
                                <td>Neurotic Cerebral somnifarous</td>
                                <td>Khas khas (Poppy seeds)</td>
                            </tr>
                            <tr>
                                <td>71. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Amphetamine</td>
                            </tr>
                            <tr>
                                <td>72. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Strichnox nuxvomica</td>
                            </tr>
                            <tr>
                                <td>73. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Dhatura Fastuossa seeds (Red)</td>
                            </tr>
                            <tr>
                                <td>74. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Myristica fragrant (jayphal-Nutmeg)</td>
                            </tr>
                            <tr>
                                <td>75. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Physostigina Vanenosum</td>
                            </tr>
                            <tr>
                                <td>76. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Methaqualone &amp; Diphashy oramy combination Mandrax-drimdolls</td>
                            </tr>
                            <tr>
                                <td>77. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Diazepam</td>
                            </tr>
                            <tr>
                                <td>78. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Dhatura Fastuossa seeds (Black)</td>
                            </tr>
                            <tr>
                                <td>79. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Camphor cinnamom (Kapoor)</td>
                            </tr>
                            <tr>
                                <td>80. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Afim (Extracted from Poppy capsule)</td>
                            </tr>
                            <tr>
                                <td>81. </td>
                                <td>Spinal poison</td>
                                <td>Strychnine</td>
                            </tr>
                            <tr>
                                <td>82. </td>
                                <td>Neurotic cerebral Deliriant</td>
                                <td>Dhatura Fastuose (Fruit)</td>
                            </tr>
                            <tr>
                                <td>83. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Hyosciamus niger</td>
                            </tr>
                            <tr>
                                <td>84. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Benzene</td>
                            </tr>
                            <tr>
                                <td>85. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Ethyl Alcohol</td>
                            </tr>
                            <tr>
                                <td>86. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Chloroform</td>
                            </tr>
                            <tr>
                                <td>87. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Baygon(carbamates)</td>
                            </tr>
                            <tr>
                                <td>88. </td>
                                <td>Cerebral Deliriant</td>
                                <td>Bhang(dry)</td>
                            </tr>
                            <tr>
                                <td>89. </td>
                                <td>Cerebral Deliriant</td>
                                <td>Bhang (Green Plant)</td>
                            </tr>
                            <tr>
                                <td>90. </td>
                                <td>Neurotic Cerebral Analgesic</td>
                                <td>Petiihidine</td>
                            </tr>
                            <tr>
                                <td>91. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Chloral hydrate</td>
                            </tr>
                            <tr>
                                <td>92. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Napthaline balls</td>
                            </tr>
                            <tr>
                                <td>93. </td>
                                <td>Cerebral Inebriant</td>
                                <td>Barbiturate</td>
                            </tr>
                            <tr>
                                <td>94. </td>
                                <td>Cardiac Poison</td>
                                <td>Tobacco(Nicotiana Tabacum)</td>
                            </tr>
                            <tr>
                                <td>95. </td>
                                <td>Cardiac Poison</td>
                                <td>Aconitum</td>
                            </tr>
                            <tr>
                                <td>96. </td>
                                <td>Cardiac Poison</td>
                                <td>Areca Catechu(Beetle Nut)</td>
                            </tr>
                            <tr>
                                <td>97. </td>
                                <td>Cardiac Poison</td>
                                <td>Nerium Odorum (Kaner White)</td>
                            </tr>
                            <tr>
                                <td>98. </td>
                                <td>Cardiac Poison</td>
                                <td>Yellow lever (Kaner yellow)</td>
                            </tr>
                            <tr>
                                <td>99. </td>
                                <td>Cardiac Poison</td>
                                <td>Karuwa Badam</td>
                            </tr>
                            <tr>
                                <td>100</td>
                                <td>Cardiac Poison</td>
                                <td>Nerium Odorum (kaner Red)</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <tbody>
                            <tr>
                                <td colspan="2">
                                    <center><b>List of Equipment/Article in the department of Forensic Medicine</b></center>
                                </td>
                            </tr>
                            <tr>
                                <td>S.No.</td>
                                <td>Article Name</td>
                            </tr>
                            <tr>
                                <td>1</td>
                                <td>Multimedia Projector , CPU &amp; Projection Screen</td>
                            </tr>
                            <tr>
                                <td>2</td>
                                <td>Television, DVD Player</td>
                            </tr>
                            <tr>
                                <td>3</td>
                                <td>Digital SLR Camera With Accessories</td>
                            </tr>
                            <tr>
                                <td>4</td>
                                <td>Microscope Student Type</td>
                            </tr>
                            <tr>
                                <td>5</td>
                                <td>X-Ray View Box Two In One</td>
                            </tr>
                            <tr>
                                <td>6</td>
                                <td>Stop Watch</td>
                            </tr>
                            <tr>
                                <td>7</td>
                                <td>Anthropometric Set including</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>A) Folding Metal Rod Upto 7 Ft</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>B) Osteometric Board</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>C) Craniometer</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>D) Mandibulometer</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>E) Goniometer</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>F) Vernier Calipers</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>G) Equipment for Reporting Height</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>H) Weighing Machine Dial Type Human</td>
                            </tr>
                            <tr>
                                <td>8</td>
                                <td>Digital pH Meter</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>Digital Spectrophotometer</td>
                            </tr>
                            <tr>
                                <td>10</td>
                                <td>Chemical Balance</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>Distillation Plant</td>
                            </tr>
                            <tr>
                                <td>12</td>
                                <td>Refrigerator</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>Centrifuge</td>
                            </tr>
                            <tr>
                                <td>14</td>
                                <td>Slide Warming Table</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>Hot Plate</td>
                            </tr>
                            <tr>
                                <td>16</td>
                                <td>Glass Cutting Pencil</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>Spectroscopic Lens With Adjustable Slit</td>
                            </tr>
                            <tr>
                                <td>18</td>
                                <td>Dissection Set Complete</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>Digital BP Instrument</td>
                            </tr>
                            <tr>
                                <td>20</td>
                                <td>Stethoscope</td>
                            </tr>
                            <tr>
                                <td>21</td>
                                <td>Charts and Models</td>
                            </tr>
                            <tr>
                                <td>(B)</td>
                                <td>Research</td>
                            </tr>
                            <tr>
                                <td>22</td>
                                <td>Binocular Research Type With Attachment For Camera</td>
                            </tr>
                            <tr>
                                <td>23</td>
                                <td>Deep Freezer For Keeping Tissue</td>
                            </tr>
                            <tr>
                                <td>24</td>
                                <td>Automatic Tissue Processing Machine</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>Following can be shared with Central Research Lab:</td>
                            </tr>
                            <tr>
                                <td>25</td>
                                <td>Microtome With Knife</td>
                            </tr>
                            <tr>
                                <td>26</td>
                                <td>Paraffin Bath Embedding</td>
                            </tr>
                            <tr>
                                <td>27</td>
                                <td>Water Bath For Tissue Floatation</td>
                            </tr>
                            <tr>
                                <td>28</td>
                                <td>L Modes</td>
                            </tr>
                            <tr>
                                <td>29</td>
                                <td>Block Holder</td>
                            </tr>
                            <tr>
                                <td>30</td>
                                <td>Abrasive Powder</td>
                            </tr>
                            <tr>
                                <td>31</td>
                                <td>Thin Layer Chromatograph</td>
                            </tr>
                            <tr>
                                <td>32</td>
                                <td>Gas Chromatograph</td>
                            </tr>
                            <tr>
                                <td>33</td>
                                <td>Ultra Violet Spectroscope</td>
                            </tr>
                            <tr>
                                <td>34</td>
                                <td>Infra Red Spectroscope</td>
                            </tr>
                            <tr>
                                <td>35</td>
                                <td>Mass Spectrometer</td>
                            </tr>
                            <tr>
                                <td>(C)</td>
                                <td>Medico legal work</td>
                            </tr>
                            <tr>
                                <td>36</td>
                                <td>Cold Storage For Dead Bodies</td>
                            </tr>
                            <tr>
                                <td>37</td>
                                <td>Weighing Machine For Dead Bodies</td>
                            </tr>
                            <tr>
                                <td>38</td>
                                <td>Autopsy Tables</td>
                            </tr>
                            <tr>
                                <td>39</td>
                                <td>Stryker Type Autopsy Saw With Accessories</td>
                            </tr>
                            <tr>
                                <td>40</td>
                                <td>Weighing Machine For Organs</td>
                            </tr>
                            <tr>
                                <td>41</td>
                                <td>Weighing Machine For Fetus</td>
                            </tr>
                            <tr>
                                <td>42</td>
                                <td>Dissection Set Complete</td>
                            </tr>
                            <tr>
                                <td>43</td>
                                <td>Brain Knife</td>
                            </tr>
                            <tr>
                                <td>44</td>
                                <td>Hack Saw</td>
                            </tr>
                            <tr>
                                <td>45</td>
                                <td>Rib Shear Left &amp; Right</td>
                            </tr>
                            <tr>
                                <td>46</td>
                                <td>Measuring Tape( Steel Tape Roll)</td>
                            </tr>
                            <tr>
                                <td>47</td>
                                <td>Magnifying Lens</td>
                            </tr>
                            <tr>
                                <td>48</td>
                                <td>X- Ray View Box (4 In 1)</td>
                            </tr>
                            <tr>
                                <td>49</td>
                                <td>Tooth Extractor Left &amp; Right</td>
                            </tr>
                            <tr>
                                <td>50</td>
                                <td>Hand Set Heat Sealer</td>
                            </tr>
                            <tr>
                                <td>51</td>
                                <td>Instrument Trolley</td>
                            </tr>
                            <tr>
                                <td>52</td>
                                <td>Stretchers for shifting dead bodies</td>
                            </tr>
                            <tr>
                                <td>53</td>
                                <td>Rectal Thermometer</td>
                            </tr>
                            <tr>
                                <td>54</td>
                                <td>Portable X-ray Machine (can be shared with Radiology Department)</td>
                            </tr>
                            <tr>
                                <td>(D)</td>
                                <td>Consumable for medico legal work</td>
                            </tr>
                            <tr>
                                <td>55</td>
                                <td>Disposable Cap-Mask-Gown, Gloves Etc.</td>
                            </tr>
                            <tr>
                                <td>56</td>
                                <td>Cotton Rolls, Suture Materials Etc.</td>
                            </tr>
                            <tr>
                                <td>57</td>
                                <td>Formalin, Rectified Spirit,Lugol&rsquo;s Iodine</td>
                            </tr>
                            <tr>
                                <td>58</td>
                                <td>Swabs, Preservative Bulbs etc.</td>
                            </tr>
                            <tr>
                                <td>59</td>
                                <td>Viscera Bottles etc.</td>
                            </tr>
                            <tr>
                                <td>60</td>
                                <td>Syringe (5, 10, 50 ml)</td>
                            </tr>
                            <tr>
                                <td>61</td>
                                <td>Preservative Salts</td>
                            </tr>
                            <tr>
                                <td>62</td>
                                <td>Packing and Sealing materials</td>
                            </tr>
                            <tr>
                                <td>63</td>
                                <td>Victim examination table (Gynecological Examination table with leg stirrups)</td>
                            </tr>
                            <tr>
                                <td>64</td>
                                <td>Screen</td>
                            </tr>
                            <tr>
                                <td>65</td>
                                <td>OT light Shadowless adjustable</td>
                            </tr>
                            <tr>
                                <td>66</td>
                                <td>Hymenoscope</td>
                            </tr>
                            <tr>
                                <td>67</td>
                                <td>Colposcope</td>
                            </tr>
                            <tr>
                                <td>68</td>
                                <td>Sims Speculum</td>
                            </tr>
                            <tr>
                                <td>69</td>
                                <td>Cuscos Speculum</td>
                            </tr>
                            <tr>
                                <td>70</td>
                                <td>Sponge holding forceps</td>
                            </tr>
                            <tr>
                                <td>71</td>
                                <td>Toothed forceps</td>
                            </tr>
                            <tr>
                                <td>72</td>
                                <td>Ovum forceps</td>
                            </tr>
                            <tr>
                                <td>73</td>
                                <td>Proctoscope</td>
                            </tr>
                            <tr>
                                <td>74</td>
                                <td>Torch</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td>[*] Mandatory for the colleges where sexual assault examination done in FMT Department but not applicable to colleges where sexual assault examination is not done in FMT</td>
                            </tr>
                            <tr>
                                <td>75</td>
                                <td>Desktop Computer</td>
                            </tr>
                            <tr>
                                <td>76</td>
                                <td>Laptop</td>
                            </tr>
                            <tr>
                                <td>77</td>
                                <td>Printer with Scanner</td>
                            </tr>
                            <tr>
                                <td>78</td>
                                <td>Photocopier machine</td>
                            </tr>
                            <tr>
                                <td colspan="2">
                                    <center><b>LIST OF EQUIPMENT IN FORENSIC MEDICINE DEPARTMENT</b></center>
                                </td>
                            </tr>
                            <tr>
                                <td>S.No.</td>
                                <td>Article Name</td>
                            </tr>
                            <tr>
                                <td>1</td>
                                <td>Anthropometric Set</td>
                            </tr>
                            <tr>
                                <td>2</td>
                                <td>B. P. Apparatus</td>
                            </tr>
                            <tr>
                                <td>3</td>
                                <td>Block Holder</td>
                            </tr>
                            <tr>
                                <td>4</td>
                                <td>Brain Knife</td>
                            </tr>
                            <tr>
                                <td>5</td>
                                <td>Chemical Balance</td>
                            </tr>
                            <tr>
                                <td>6</td>
                                <td>Deep freezer for keeping tissue</td>
                            </tr>
                            <tr>
                                <td>7</td>
                                <td>Digital Ph meter</td>
                            </tr>
                            <tr>
                                <td>8</td>
                                <td>Digital Spectrometer</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>Distillation plant</td>
                            </tr>
                            <tr>
                                <td>10</td>
                                <td>Electric Auto Slide projector</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>Glass cutting pencil</td>
                            </tr>
                            <tr>
                                <td>12</td>
                                <td>Hand set Heat Sealer</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>Hot plates</td>
                            </tr>
                            <tr>
                                <td>14</td>
                                <td>L. Modes</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>Leather Strops</td>
                            </tr>
                            <tr>
                                <td>16</td>
                                <td>Microscope Binoculer, Research</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>Microscope student type</td>
                            </tr>
                            <tr>
                                <td>18</td>
                                <td>Microtome with Knife</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>Paraffin Bath Embedding</td>
                            </tr>
                            <tr>
                                <td>20</td>
                                <td>Refrigerator</td>
                            </tr>
                            <tr>
                                <td>21</td>
                                <td>Slide Warming table</td>
                            </tr>
                            <tr>
                                <td>22</td>
                                <td>SLR Camera with accessories</td>
                            </tr>
                            <tr>
                                <td>23</td>
                                <td>Spectroscope Lens with adjustable slit</td>
                            </tr>
                            <tr>
                                <td>24</td>
                                <td>Stone Carbarandon</td>
                            </tr>
                            <tr>
                                <td>25</td>
                                <td>Stopwatch</td>
                            </tr>
                            <tr>
                                <td>26</td>
                                <td>Water Bath (Tissue Flowing)</td>
                            </tr>
                            <tr>
                                <td>27</td>
                                <td>Oseometric boards</td>
                            </tr>
                            <tr>
                                <td>28</td>
                                <td>Stethoscope&nbsp;</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </section>
    </div>
</div>
</div>

@endsection