...
@endif
    ...
    <div class="card-body">
      ...
      <!-- Multi Columns Form -->
      <form  method="POST" action="{{route('naac_metric.edit',['id' => $metric->metric_id])}}" enctype="multipart/form-data" >
        @csrf
...
<option {{$criterion->criterion_id  == $metric->criterion_id ? 'selected' : ''}} value="{{$criterion->criterion_id}}">{{$criterion->criterion_name}}</option>
                @endforeach
                  </select>
            ...
            <div class="col-md-3">
              ...
              <div class="col-md-3">
                ...
                <input type="text" name="metric_name" id="metric_name" class="form-control" value="{{$metric->metric_name}}" required />
            </div>

        <div class="col-md-3">
            <label for="metric_description" class="form-label" style="font-size:13px;">Metric Description</label>
            <input type="text" name="metric_description" id="metric_description" class="form-control" value="{{$metric->metric_description}}" required />
        </div>
...
<div class="row mt-3">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select" name="metric_status" id = "metric_status" aria-label="Default select example" required>
            ...
            <option {{$metric->metric_status == 'Y' ? 'selected' : ''}} value="Y">Yes</option>
            <option {{$metric->metric_status == 'N' ? 'selected' : ''}} value="N">No</option>
          </select>
      ...
      <div class="col-md-6"> 
        ...
        <input class="form-control" type="number" name="metric_priority" id="metric_priority"  value="{{$metric->metric_priority}}" required />
      </div>
        ...
        <div class="text-center"><br/>
          <input type="submit" class="btn btn-primary" value="Update Metric" id="myButton"  />
...
<?php if (isset($metric->key_indicator_id)): ?>
const key_indicator_id_new = "<?php echo $metric->key_indicator_id; ?>";
<?php endif; ?>
...
window.onload = function () {
    const criterionID = $("#criterion_id").val(); // Get selected criterion ID
    const keyIndicatorSelect = $('#key_indicator_id');
