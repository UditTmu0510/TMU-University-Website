    ...
    try {
        ...
        if (cachedData) {
            ...
            const response = await fetch(
                "http://127.0.0.1:8000/menubarData.json"
            );
...
function showMenuContent(choice, listItem) {
    ...
    if (
        listItem
            .querySelector("h1")
            .classList.contains("underline__effect_permanent")
    ) {
        listItem
            .querySelector("h1")
            .classList.remove("underline__effect_permanent");
...
async function openSubMenuContent(value, menu, listItem) {
    try {
        ...
        const response = await fetch("http://127.0.0.1:8000/menubarData.json");
...
async function openSubMenuContentMobile(value, menu) {
    try {
        ...
        for (let item in catalogue[value][menu]) {
            if (catalogue[value][menu][item].link) {
                htmlContent += `<li><a href="${catalogue[value][menu][item].link}" class="additional-link">${item}</a><i class="bi bi-arrow-right-short"></i></li>`;
            } else {
    ...
    if (h1Element.classList.contains("underline__effect_permanent")) {
        ...
        liElements.forEach((li) => {
            ...
            if (nestedUl) {
                ...
                if (
                    hElement.classList.contains("underline__effect_permanent")
                ) {
                    // If the h1 element has the permanent underline class, remove it
        ...
        if (categoryList[value] && typeof categoryList[value] === "object") {
            ...
            for (const itemName in category) {
                if (!category[itemName].link) {
                    ...
                    htmlContent += `<li><a href="${category[itemName].link}" class="additional-link">${itemName}</a><i class="bi bi-arrow-right-short"></i></li>`;
                }
            }
        }
